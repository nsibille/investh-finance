...
**Page de détail d'un achat (`/achats/[id]`)** — `card-surface` : en-tête (titre `text-2xl` + badges `badge-group`/`badge-status-*` + pastille catégorie) et barre d'actions (`btn-secondary-sm` Modifier, `btn-ghost-sm` Retirer du groupe / Archiver / Supprimer) → encart KPI (`--color-bg-subtle`) Total dépensé · Budget prévu · Reste à payer · Transactions (mono `amount-lg`) → `purchase-galaxy` en `variant="page"` → `btn-ghost-sm` Ajouter un achat au groupe. Réutilise `PurchaseForm` + `InstallmentEditor` (modale) et `modal-confirm-danger` (suppression). Implémentation : `src/components/purchases/PurchaseDetailView.tsx`.
